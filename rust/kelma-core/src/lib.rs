...
fn dispatch_session(session: &KelmaSession, op: &str, input: &[u8]) -> Result<String, String> {
    ...
    let value = match op {
        ...
        "writeCardHtml" => session.write_card_html(&request)?,
        ...
        "suspendCard" => session.suspend_card(&request)?,
        "unsuspendCard" => session.unsuspend_card(&request)?,
        "buryCard" => session.bury_card(&request)?,
        "setCardFlag" => session.set_card_flag(&request)?,
        "setCardDeck" => session.set_card_deck(&request)?,
        "deleteCard" => session.delete_card(&request)?,
        "toggleCardMark" => session.toggle_card_mark(&request)?,
        "getNoteEdit" => session.get_note_edit(&request)?,
        "updateNote" => session.update_note(&request)?,
        "notetypes" => session.notetypes()?,
        "addNote" => session.add_note(&request)?,
        "checkNoteFields" => session.check_note_fields(&request)?,
        "exportDeck" => session.export_deck(&request)?,
        "importApkg" => session.import_apkg(&request)?,
        ...
        "syncMediaStart" => session.sync_media_start(&request)?,
        ...
        "resetForV2Restore" => session.reset_for_v2_restore()?,
        "resetMedia" => session.reset_media()?,
